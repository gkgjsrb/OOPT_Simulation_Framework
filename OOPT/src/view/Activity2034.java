...
public class Activity2034 extends JTabbedPane {

	/**
	 * Create the panel.
	 */
	DefaultTableModel model;
	public Activity2034(JTree tree, ArrayList<Glossary> gl, Datainfo data) {
		gl.add(new Glossary("R"));	
		String[] colName= {"Term","Category","Remarks"};
		...
		button_2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				for(int i = 0; i < table.getRowCount(); i++) {
					Glossary g = new Glossary("R");
					g.setTerm((String)table.getValueAt(i, 0));
					g.setCategory((String)table.getValueAt(i, 1));
	...
	public void open(ArrayList<Glossary> gl) {
		...
		model.setRowCount(0);
		for(Glossary g : gl) {
			Object[] add= {g.getTerm(), g.getCategory(), g.getRemarks()};
